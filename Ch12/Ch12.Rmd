...
# STAN was blowing up, but today (12/8) it works!
# 
# tat.data <- list(nSubj = 40,
#                  nCond = 2,
#                  N = rep(10, 40),
...
#     # Prior on theta
#     theta[i] ~ dbeta( a[Cond[i]] , b[Cond[i]])
#     }
#   for ( i in 1 : nCond ) {
#     a[i] <- mu[i] * kappa[i]
#     b[i] <- (1-mu[i]) * kappa[i]
...
#   }
# 
# jagsfit <- jags(data = tat.data, model.file = tat.mod, inits = inits, parameters.to.save = c('theta', 'mu', 'kappa', 'a', 'b'), n.chains = 3, n.iter = 5000, n.burnin = 500, n.thin = 3)
