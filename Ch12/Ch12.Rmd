...
tat.model <-'
  ...
  transformed parameters{
    ...
    vector[nSubj] b;
    for (i in 1 : nSubj){
      a[i] <- mu[cond] * kappa[cond];
      b[i] <- 1 - mu[cond] * kappa[cond];
    };
  }

  model{
    for ( i in 1 : nSubj)
      z[nSubj] ~ binomial(N, theta);
    theta ~ beta(a, b);
    for (i in 1 : nCond){
      mu[i] ~ beta(3.6, 6.5);
...
test <- stan(model_code = tat.model, data = tat.data, iter = 10000, chains = 3)
